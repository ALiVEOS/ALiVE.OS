...
switch (_operation) do {
	...
	case "openMenu": {
		...
		if (count _authorized > 0 && !((getPlayerUID player in _authorized) || (typeOf player in _authorized) || (name player in _authorized) || (faction player in _authorized))) exitWith {["You are not authorized to speak with civilians."] call ALiVE_fnc_dumpR;};
