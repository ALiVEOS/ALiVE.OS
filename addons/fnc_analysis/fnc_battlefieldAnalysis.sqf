...
switch(_operation) do {
    ...
    case "LOGISTICS_INSERTION": {
        ...
        _args params ["_eventID","_eventData"];

        _eventData params ["_position","_faction","_side","_logEventID"];

        private _eventsInProgress = [_logic, "eventsInProgress"] call ALIVE_fnc_hashGet;

        private _logisticsEvent = [] call ALIVE_fnc_hashCreate;
    ...
    case "LOGISTICS_DESTINATION": {
        ...
        if(_logEventID in (_eventsInProgress select 1)) then {
            private _logisticsEvent = [_eventsInProgress,_logEventID] call ALIVE_fnc_hashGet;
    ...
    case "PROFILE_KILLED": {
        ...
        _eventData params ["_position","_faction","_side"];

        private _eventSector = [ALIVE_sectorGrid, "positionToSector", _position] call ALIVE_fnc_sectorGrid;
        private _eventSectorID = [_eventSector,"id"] call ALIVE_fnc_hashGet;

        private _sectorData = [_eventSector,"data"] call ALIVE_fnc_hashGet;

        if (isnil "_sectorData") exitwith {};

        if!("casualties" in (_sectorData select 1)) then {
            private _casualties = [] call ALIVE_fnc_hashCreate;
        ...
        private _casualties = [_sectorData,"casualties"] call ALIVE_fnc_hashGet;
        private _factionCasualties = [_casualties,"faction"] call ALIVE_fnc_hashGet;
        private _sideCasualties = [_casualties,"side"] call ALIVE_fnc_hashGet;
        ...
        private _factionCasualtyCount = [_factionCasualties,_faction] call ALIVE_fnc_hashGet;
        _factionCasualtyCount = _factionCasualtyCount + 1;
        [_factionCasualties,_faction,_factionCasualtyCount] call ALIVE_fnc_hashSet;

        private _sideCasualtyCount = [_sideCasualties,_side] call ALIVE_fnc_hashGet;
        ...
        private _casualtySectors = [_logic, "casualtySectors"] call ALIVE_fnc_hashGet;
    ...
    case "OPCOM_RECON": {
        ...
        private _side = _eventData select 0;
        private _position = _eventData select 1 select 2 select 1;
        private _size = _eventData select 1 select 2 select 2;
        private _type = _eventData select 1 select 2 select 3;
        private _priority = _eventData select 1 select 2 select 4;
        private _clusterID = _eventData select 1 select 2 select 6;
    ...
    case "storeClusterEventToSector": {
        ...
        _args params ["_clusterID","_eventData"];

        private _type = _eventData select 0;
        private _position = _eventData select 2;
        private _side = _eventData select 3;
        private _clusterType = _eventData select 4;
        private _size = _eventData select 5;
        private _priority = _eventData select 6;
        ...
        private _activeClusters = [_sectorData,"activeClusters"] call ALIVE_fnc_hashGet;

        if!(_clusterID in (_activeClusters select 1)) then {
            private _activeCluster = [] call ALIVE_fnc_hashCreate;
        ...
        private _activeCluster = [_activeClusters,_clusterID] call ALIVE_fnc_hashGet;
        ...
        private _activeSectors = [_logic, "activeSectors"] call ALIVE_fnc_hashGet;
    ...
    case "getClustersOwnedBySide": {
        ...
        {
            private _sectorData = [_x,"data"] call ALIVE_fnc_hashGet;

            if !(isnil "_sectorData") then {
                private _clusters = [_sectorData,"activeClusters"] call ALIVE_fnc_hashGet;
    ...
    case "getSectorsContainingSide": {
        ...
        private _side = _args select 0;
        private _sectorsContainingSide = [];
...

        {
            ...
            if (!isnil "_sectorData" && {"entitiesBySide" in (_sectorData select 1)}) then {
                private _entities = [_sectorData,"entitiesBySide"] call ALIVE_fnc_hashGet;
                private _sideEntities = [_entities,_side] call ALIVE_fnc_hashGet;
    ...
    case "getSectorsContainingSideVehicles": {
...

        {
            ...
            if (!isnil "_sectorData" && {"vehiclesBySide" in (_sectorData select 1)}) then {
                private _vehicles = [_sectorData,"vehiclesBySide"] call ALIVE_fnc_hashGet;
                private  _sideVehicles = [_vehicles,_side] call ALIVE_fnc_hashGet;
