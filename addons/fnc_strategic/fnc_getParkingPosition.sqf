...
if !(!isnil "_vehicleClass" && {!isnil "_building"}) exitwith {
    ["getParkingPosition didn't receive fitting input: %1! Exiting...",_this] call ALiVE_fnc_dumpR;
};
...
for "_i" from 1 to 4 do {
    ...
    if (_isEmpty) exitwith {
        if(_debug) then {
            ["getParkingPosition found empty parking position at %1 in direction %2",_position,_direction] call ALiVE_fnc_dumpR;
        };
    };
};
...
if (!_isEmpty || {isNil "_position"} || {count _position < 2}) exitwith {
        if(_debug) then {
            ["getParkingPosition didn't find any suiting parking position!",_position,_direction] call ALiVE_fnc_dumpR;
        };
...
if (!_isEmpty) then {
    ...
    if(_debug) then {
        ["getParkingPosition Failed! Assigned failsafe pos at %1 in direction %2",_position,_direction] call ALiVE_fnc_dumpR;
...
if(_debug) then {
    ["getParkingPosition result is _pos %1 | _dir %2",_position,_direction] call ALiVE_fnc_dumpR;
