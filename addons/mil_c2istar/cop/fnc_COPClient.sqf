...
["info", "client", "Player side cached: %1 (player: %2)", [ALIVE_COP_playerSideKey, name player]] call ALIVE_fnc_COPLog;

// ============================================================================
// RESPAWN HANDLER — refresh the side cache
// ============================================================================
// Must use the MISSION-level "PlayerRespawned" event, not a per-unit "Respawn"
// event. The per-unit handler fires on the OLD unit's corpse and doesn't
// transfer to the new player unit, so on a second respawn the side cache
// would never refresh. The mission event fires on every player respawn with
// the new unit as the first parameter — exactly what we need.
if (isNil "ALIVE_COP_RespawnMEHIndex") then {
    ALIVE_COP_RespawnMEHIndex = addMissionEventHandler ["PlayerRespawned", {
        params ["_newUnit", "_oldUnit", "_respawnDelay"];
    ...
    ["debug", "client", "PlayerRespawned mission handler registered (index %1)", [ALIVE_COP_RespawnMEHIndex]] call ALIVE_fnc_COPLog;
};
