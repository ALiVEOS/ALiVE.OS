...
ALIVE_fnc_COPDrawBftMarker = {
    ...
    // Side at index 1 (matches enemy spotrep convention). For multi-side
    // copDisplaySides configs, each entry carries its own side so the
    // marker color resolves to the contact's side, not the player's.
    // _faction defaults to "" so a stale broadcast from a pre-faction
    // server build (older 5-tuple shape) still destructures cleanly.
    _entry params ["_pos", "_sideKey", "_type", "_count", "_sizeInd", ["_faction", "", [""]]];
    ...
    // Label format mirrors the enemy marker (TYPE xN [FACTION]) for visual
    // parity. Age suffix is omitted — BFT is continuous-track, not aged
    // intel. Type + count + faction match the enemy data set so a glance
    // at the map reads the same on both sides.
    private _label = "";
    if (ALIVE_COP_render_showBftDetail) then {
        _label = toUpper _type;
        if (_count > 1) then { _label = format ["%1 x%2", _label, _count]; };
        if (ALIVE_COP_FEAT_FACTION && {_faction != ""}) then {
            _label = format ["%1 [%2]", _label, _faction];
        };
    };
...
ALIVE_fnc_COPDrawAll = {
    ...
    // PublicVariable channel names — list pre-built at client init from the
    // copDisplaySides Eden attribute (CSV of side keys). For the default
    // single-side case the list has one entry — same per-frame cost as the
    // prior single-variable read. For multi-side configurations each pool
    // is concatenated; dedup is deliberately NOT applied across feeds
    // because a unit known to two friendly OPCOMs is genuinely two intel
    // observations — visually overlapping markers signal cross-confirmed
    // contacts.
    private _intel = [];
    private _bft   = [];
    private _obj   = [];
    {
        _intel append (missionNamespace getVariable [_x, []]);
    } forEach ALIVE_COP_intelVars_intelList;
    {
    ...
    if (ALIVE_COP_LAYER_BFT) then {
        {
            ...
            if (_bypassAnchor || {(_entryPos distance2D _anchor) <= _anchorDist}) then {
                ...
                if (_ex >= (_viewMinX - 200) && {_ex <= (_viewMaxX + 200)}
                    && {_ey >= (_viewMinY - 200) && {_ey <= (_viewMaxY + 200)}}) then {
                    // _sideKey now lives inside _x (entry index 1) — see
                    // COPBuildBFT's cluster reduce. Renderer pulls it from
                    // the entry so multi-side display configurations color-
                    // code per-side correctly rather than via the player's
                    // own side.
                    [_mapCtrl, _x] call ALIVE_fnc_COPDrawBftMarker;
                };
            };
        } forEach _bft;
    };
    ...
    if (ALIVE_COP_LAYER_ENEMIES) then {
        {
            // Cross-side own-units filter: when copDisplaySides combines
            // multiple sides' OPCOM feeds, an OPCOM on a side OTHER than the
            // player's has the player's own-side units catalogued as enemy
            // contacts (e.g. EAST OPCOM tracks WEST infantry from EAST's
            // POV). Those would render as enemy markers — duplicating the
            // BFT entries already showing the same physical units. BFT is
            // authoritative for own-side units; skip enemy entries whose
            // side matches the player's so the map doesn't double-mark
            // them. Spotrep tuple index 1 is the contact's side (set in
