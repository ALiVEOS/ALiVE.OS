...
//           heading, speed]  or nil if profile invalid
// ----------------------------------------------------------------------------
ALIVE_fnc_COPBuildSpotrep = {
    params [["_profileID", "", [""]], ["_pos", [0,0,0], [[]]]];
    ...
    [_pos, _profileSide, _type, _factionCode, _count, _sizeInd, "", _heading, _speed]
};
...
ALIVE_fnc_COPClusterEnemy = {
    ...
    private _result = [];
    {
        ...
        _result pushBack [_center, _clusterSide, _dominant, _factionCode, _totalCount, _sizeInd,
                          _activity, _heading, _speed, 0, _isMixed, _trail];
    } forEach _clusters;
...
ALIVE_fnc_COPBuildBFT = {
...

    {
        ...
        _rawProfileCount = _rawProfileCount + count _nearProfiles;
        {
            ...
            if (_profileID != "" && {!(_profileID in _seen)}) then {
                ...
                private _type = [_profile] call ALIVE_fnc_COPTypeFromProfile;
                private _count = count _units max 1;
                _positions pushBack [_pos, _type, _count];
            };
    ...
    private _result = [];
    {
        ...
        _result pushBack [_center, _dominant, _totalCount, _sizeInd];
    } forEach _clusters;
...
[] spawn {
    ...
    while {ALIVE_COP_SERVER_RUNNING} do {
        ...
        if (ALIVE_COP_LAYER_ENEMIES) then {
            {
...

                {
                    ...
                    private _spotrep = [_profileID, _pos] call ALIVE_fnc_COPBuildSpotrep;
        ...
        if (_cycleMs > ALIVE_COP_DEBUG_PERF_WARN_MS) then {
            ["warn", "perf", "Loop A cycle %1 took %2ms (threshold: %3ms)", [_cycleCount, _cycleMs, ALIVE_COP_DEBUG_PERF_WARN_MS]] call ALIVE_fnc_COPLog;
        };
...
[] spawn {
    ...
    while {ALIVE_COP_SERVER_RUNNING} do {
        ...
        if (_cycleMs > ALIVE_COP_DEBUG_PERF_WARN_MS) then {
            ["warn", "perf", "Loop B cycle %1 took %2ms (threshold: %3ms)", [_cycleCount, _cycleMs, ALIVE_COP_DEBUG_PERF_WARN_MS]] call ALIVE_fnc_COPLog;
        };
