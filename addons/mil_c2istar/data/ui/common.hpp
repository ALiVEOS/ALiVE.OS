...
class C2Tablet_RscGUIListBox : C2Tablet_RscListBox {
    color[] = {1, 1, 1, 1};
...
class C2Tablet_RscMap
{
    ...
    deletable = 1; // #698 lets ALIVE_fnc_mapControlSwap ctrlDelete this map for the terrain toggle (base, so the schematic inherits it too)
    moveOnEdges = 1;
    ptsPerSquareSea = 8;
    ptsPerSquareTxt = 10;
    ptsPerSquareCLn = 10;
...
// #698 schematic (drawn-map) variant for the terrain-mode toggle - identical to C2Tablet_RscMap but
// with the satellite layer off. ALIVE_fnc_mapControlSwap swaps the map between these two.
class C2Tablet_RscMapSchematic : C2Tablet_RscMap
{
    maxSatelliteAlpha = 0;
};
