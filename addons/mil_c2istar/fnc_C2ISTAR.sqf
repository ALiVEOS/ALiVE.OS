...
switch(_operation) do {
    ...
    case "init": {
        ...
        if (isServer) then {
            ...
            // APPEND extends the stock default task list. C2ISTAR inits at a higher
            // functionPriority than the placement modules that trigger the static-data
            // load, so in a scene with no earlier (>150-priority) module the default isn't
            // loaded yet and APPEND would union against an empty registry, dropping the
            // stock list. Load static data now (synchronous; once-per-mission guarded, so
            // later module loads no-op) so the default is present before we append. The
            // REPLACE and empty-value cases don't need it.
            if ((toUpper _customTasksMode) == "APPEND" && {_customTasksValue != ""} && {isNil "ALIVE_autoGeneratedTasks"}) then {
