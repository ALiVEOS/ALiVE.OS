    ...
    case "autoGenerateTasks": {
        if (_args isEqualType []) then {
            ...
            if (!isNil "ALIVE_autoGeneratedTasks" && {count ALIVE_autoGeneratedTasks > 0} && {_countActive == 0} && {count (allPlayers - entities "HeadlessClient_F") > 0}) then {
                ...
                private _taskType = [ALIVE_autoGeneratedTasks, _taskLocation, _taskLocationType, _taskSide, _taskFaction, _tasksCurrent] call ALIVE_fnc_taskSelectAutoGeneratedType;
