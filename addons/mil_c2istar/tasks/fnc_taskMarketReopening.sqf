...
SCRIPT(taskMarketReopening);

/* ----------------------------------------------------------------------------
Function: ALIVE_fnc_taskMarketReopening

Description:
Secure a market reopening and keep civilians safe while trade resumes.
...
switch (_taskState) do {
    case "init": {
        ...
        if (_taskID == "") exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _taskID!"] call ALiVE_fnc_Dump};
        if (_requestPlayerID == "") exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _requestPlayerID!"] call ALiVE_fnc_Dump};
        if (_taskFaction == "") exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _taskFaction!"] call ALiVE_fnc_Dump};
        if (_taskLocationType == "") exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _taskLocationType!"] call ALiVE_fnc_Dump};
        if (count _taskLocation == 0) exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _taskLocation!"] call ALiVE_fnc_Dump};
        if (count _taskPlayers == 0) exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _taskPlayers!"] call ALiVE_fnc_Dump};
        if (_taskApplyType == "") exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _taskApplyType!"] call ALiVE_fnc_Dump};
        if (_taskEnemyFaction == "") exitWith {["C2ISTAR - Task MarketReopening - Wrong input for _taskEnemyFaction!"] call ALiVE_fnc_Dump};
        ...
        if (_clusterCenter isEqualTo []) exitWith {["C2ISTAR - Task MarketReopening - Invalid cluster center!"] call ALiVE_fnc_Dump};
        ...
        for "_i" from 0 to 1 do {
            ...
            private _vip = createAgent [selectRandom ["C_man_polo_1_F", "C_man_polo_4_F", "C_man_p_fugitive_F", "C_man_w_worker_F"], _vipPosition, [], 0, "NONE"];
        ...
        for "_i" from 0 to 3 do {
            ...
            private _crowdUnit = createAgent [selectRandom ["C_man_1", "C_man_polo_2_F", "C_man_shorts_1_F", "C_man_polo_5_F", "C_man_w_worker_F"], _crowdPosition, [], 0, "NONE"];
        ...
        private _dialogOptions = [ALIVE_generatedTasks, "MarketReopening"] call ALIVE_fnc_hashGet;
        ...
        private _taskSource = format ["%1-MarketReopening-Parent", _taskID];
        ...
        _taskSource = format ["%1-MarketReopening-Setup", _taskID];
        ...
        _taskSource = format ["%1-MarketReopening-Secure", _taskID];
        ...
        [_taskParams, "supportValue", 15] call ALIVE_fnc_hashSet;
        ...
        [_taskParams, "taskType", "MarketReopening"] call ALIVE_fnc_hashSet;
        ...
        if ((_losses select 0) > 0) then {
            ...
            [
                ...
                [
                    ...
                    [_params, "taskType", "MarketReopening"] call ALIVE_fnc_hashGet,
        ...
        if (_vipLosses > 0 || {_crowdLosses > ([_params, "maxCivilianLosses", 1] call ALIVE_fnc_hashGet)}) then {
            ...
            if (_currentWave > _totalWaves && {_entityProfileIDs isEqualTo []} && {serverTime >= _holdUntil} && {[_taskPosition, _taskPlayers, _taskSide, 250] call ALIVE_fnc_taskIsAreaClearOfEnemies}) then {
                ...
                [
                    _taskPosition,
                    _taskSide,
                    [_params, "supportValue", 15] call ALIVE_fnc_hashGet,
                    [
                        [_params, "clusterID", ""] call ALIVE_fnc_hashGet,
                        [_params, "taskType", "MarketReopening"] call ALIVE_fnc_hashGet,
                        [_params, "cooldownDuration", 4200] call ALIVE_fnc_hashGet
