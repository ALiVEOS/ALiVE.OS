...
switch(_operation) do {
        ...
        case "setorders": {
                ...
                _tacomData = ["completed",[_ProfileID,_objectiveID,_orders]];
                _statements = format ["
                    [] spawn {
                        sleep (random 10);
                        (%1 getfsmvariable '_TACOM_QUEUE') pushback %2
                    }
                ",_TACOM_FSM, _tacomData];
        ...
        // #TODO: This is unused, but will need to be adapted for use with new tacom
        case "addTask": {
        ...
        case "selectordersbystate": {
                private ["_target","_state","_module"];
                ASSERT_TRUE(typeName _args == "STRING",str _args);
                ...
                private ["_DATA_TMP","_orders"];

                switch (_state) do {
                    case ("attack") : {
                        private _objectivesWithStateAttack = _objectives find { ([_x,"opcom_state"] call ALiVE_fnc_hashGet) == "sttack" };

                        if (count _objectivesWithStateAttack > 0) then {
                            private _targetObjective = _objectivesWithStateAttack select 0;

                            [_targetObjective,"opcom_orders","attack"] call AliVE_fnc_HashSet;
