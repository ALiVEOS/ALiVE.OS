...
_QS_ST_showEmptyVehicles = TRUE;								// BOOL. TRUE to mark certain unoccupied vehicles on the map. The vehicle must be assigned this variable:    <vehicle> setVariable ['QS_ST_drawEmptyVehicle',TRUE,TRUE];    Default FALSE.   Only works if  q = TRUE;
_QS_ST_iconColor_empty = [0,0.3,0.6,0.65];						// ARRAY (NUMBERS). Color of unoccupied vehicles, in RGBA. Default = [0.7,0.6,0,0.5];
_QS_ST_iconSize_empty = 250;										// NUMBER. Icon size of unoccupied vehicles, if shown.
	...
	if (!(_v isKindOf 'Man')) then {
		if (_v getVariable ['QS_ST_drawEmptyVehicle',FALSE]) then {
			if ((count (crew _v)) isEqualTo 0) then {
				_exit = TRUE;
				_c = _QS_ST_X select 78;   // _QS_ST_iconSize_empty
				_c set [3,0.65];
				if (_ms > 0.80) then {
...
_QS_fnc_iconUnits = {
	...
	if (((_di isEqualTo 1) && ((_QS_ST_X select 65))) && {(!(_QS_ST_X select 75))}) then {
		...
		if ((_QS_ST_X select 80)) then { // _QS_ST_showEmptyVehicles
			{
				if (!(_x in _au)) then {
...
_QS_fnc_iconDrawMap = {
	...
	if (diag_tickTime > (missionNamespace getVariable 'QS_ST_updateDraw_map')) then {
		...
		missionNamespace setVariable ['QS_ST_drawArray_map',([1,_QS_ST_X] call (_QS_ST_X select 46)),FALSE];  // call iconUnits
	};
			...
			if (!isNull _x) then {
				...
				if (alive _ve) then {
					...
					_m drawIcon [
						...
						_is,
						(_po select 1),
						([_ve,1,_QS_ST_X,_ms] call (_QS_ST_X select 45)), // iconText
						_sh,
						_ts,
						_tf,
						_to
...
_QS_fnc_iconDrawGPS = {
	...
	if (diag_tickTime > (missionNamespace getVariable 'QS_ST_updateDraw_gps')) then {
		...
		missionNamespace setVariable ['QS_ST_drawArray_gps',([2,_QS_ST_X] call (_QS_ST_X select 46)),FALSE]; // call iconUnits
	};
...
_QS_ST_R = [
	...
	_QS_ST_showOwnGroup,
	_QS_fnc_iconColorGroup,
	_QS_ST_iconColor_empty, // 77
	_QS_ST_iconSize_empty,  // 78
...
if (_QS_ST_X select 0) then {
	((findDisplay 12) displayCtrl 51) ctrlAddEventHandler ['Draw',(format ['_this call %1',(_QS_ST_X select 49)])]; // iconDrawMap
			...
			for '_x' from 0 to 1 step 0 do {
				if (!(_QS_display1Opened)) then {
					if (!isNull ((findDisplay 160) displayCtrl 51)) then {
						...
						((findDisplay 160) displayCtrl 51) ctrlAddEventHandler ['Draw',(format ['_this call %1',(_QS_ST_X select 49)])]; // iconDrawMap
					};
				} else {
					if (isNull ((findDisplay 160) displayCtrl 51)) then {
						_QS_display1Opened = FALSE;
				...
				if (!(_QS_display2Opened)) then {
					if (!isNull((findDisplay -1) displayCtrl 500)) then {
						...
						((findDisplay -1) displayCtrl 500) ctrlAddEventHandler ['Draw',(format ['_this call %1',(_QS_ST_X select 49)])]; // iconDrawMap
					};
				} else {
					if (isNull ((findDisplay -1) displayCtrl 500)) then {
						_QS_display2Opened = FALSE;
