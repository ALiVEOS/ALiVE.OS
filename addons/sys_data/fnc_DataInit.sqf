...
if (isDedicated || (isServer && _pns)) then {
    ...
    // Saved data is kept per map now (ALiVE_fnc_storeKeys). Move this mission's saves
    // from the old names before anything loads them. Every loader builds its name from
    // GROUP_ID, set just above, and every module that loads saves starts at a later
    // priority than this one. The move runs unscheduled so that no load can slip in
    // halfway through it. Names only a module can build are moved when that module
    // starts (ALiVE_fnc_storeMigrate).
    if (_pns) then {
        isNil { [] call ALiVE_fnc_storeMigrate };
    } else {
        ["[ALiVE Data] Cloud saves are now named per map, so documents saved under the old names will not be found"] call ALiVE_fnc_dump;
    };
    ...
    // Saved data is kept per map (ALiVE_fnc_storeKeys): the module saves under the base
    // name, and the mission date and player saves under _DATA.
    private _missionBase = ([""] call ALiVE_fnc_storeKeys) select 0;
    if (_pns && {!(isNil {profileNameSpace getvariable _missionBase}) || {!(isNil {profileNameSpace getvariable (_missionBase + "_DATA")})}}) then {
    ...
    // Set dictionary name
    _dictionaryName = "dictionary_" + _missionBase;
    ...
    if (!_pns) then {
        ...
        if ( typeName _response != "STRING") then {
            ...
            _i = 1;
            while {_dictionaryName = format ["dictionary_%1_%2", _missionBase, _i]; _newresponse = [GVAR(datahandler), "read", ["sys_data", [], _dictionaryName]] call ALIVE_fnc_Data; typeName _newresponse != "STRING"} do {
    ...
    if (GVAR(dictionaryLoaded) && (MOD(sys_data) getVariable ["saveDateTime","false"] == "true")) then {
        ...
        _missionName = _missionBase + "_DATA";
    ...
    if (GVAR(dictionaryLoaded) && (MOD(sys_data) getVariable ["saveCompositions","false"] == "true")) then {
        ...
        _missionName = _missionBase + "_COMPOSITIONS";
        _response = [GVAR(datahandler), "read", ["sys_data", [], _missionName]] call ALIVE_fnc_Data;
        if ( typeName _response != "STRING") then {
