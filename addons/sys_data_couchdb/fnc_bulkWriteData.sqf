...
if(ALiVE_SYS_DATA_DEBUG_ON) then {
    ["SYS_DATA_COUCHDB - BULK WRITE DATA: (%1) %2",[str(_string)] call CBA_fnc_strLen,_string] call ALiVE_fnc_dump;
};
...
if(ALiVE_SYS_DATA_DEBUG_ON) then {
    ["SYS_DATA_COUCHDB - BULK WRITE DATA RESULT: %1",_response] call ALiVE_fnc_dump;
};
