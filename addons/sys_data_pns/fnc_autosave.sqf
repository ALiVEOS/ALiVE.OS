...
if !(isServer && {!isNil "ALiVE_SYS_DATA_SOURCE"} && {ALiVE_SYS_DATA_SOURCE == "pns"}) exitWith {["ALiVE SYS DATA PNS - Local machine is not the server or local save not available! Exiting..."] call ALiVE_fnc_Dump};
...
ALiVE_SYS_DATA_PNS_AUTOSAVE = _interval spawn {
    ...
    while {_interval > 0} do {
	    ...
	    sleep 1;
        ["ALiVE IS SAVING..."] caLL ALiVE_fnc_DumpMPH;
        sleep 1;
                		...
                		
	    {
			...
			if (hasInterface) then {
			    ...
			    if !(isNil "ALiVE_sys_player") then {
                    ["ALiVE SYS DATA PNS Autosave - Save Player Data id: %1 name: %2 uid: %3",_id,_name,_uid] call ALIVE_fnc_dump;
			        
                    ["ALiVE SYS DATA PNS Autosave - Player Data OPD"] call ALIVE_fnc_dump;

			         if (ALiVE_sys_player getvariable "saveLoadout") then {
			                private ["_gearHash","_unit"];
                            
                            ["ALiVE SYS DATA PNS Autosave - Storing Player Gear"] call ALIVE_fnc_dump;
				...
				if (["ALiVE_sys_profile"] call ALiVE_fnc_isModuleAvailable) then {
					...
					["ALiVE SYS DATA PNS Autosave - Player Profile Handler OPD"] call ALIVE_fnc_dump;
	    ...
	    sleep 0.2;
        ["ALiVE SAVING FINISHED!"] caLL ALiVE_fnc_DumpMPH;
	    sleep 2;
	    TitleText ["","PLAIN"];
    };
    
    ["ALiVE SYS DATA PNS - AutoSave (local) process exiting... (%1)",ALiVE_SYS_DATA_PNS_AUTOSAVE] call ALIVE_fnc_dump;
