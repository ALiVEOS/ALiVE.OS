...
=======
        [_state,"factions", [_logic,"loadFactionDataFromConfig"] call MAINCLASS] call ALiVE_fnc_hashSet;
        [_state,"customUnits", [] call ALiVE_fnc_hashCreate] call ALiVE_fnc_hashSet;
...
=======
        ...
        [_state,"editVehicle_selectedCrewBySlot", [] call ALiVE_fnc_hashCreate] call ALiVE_fnc_hashSet;
...
=======
        ...
        */
        ...
        // #TODO: If this is unused, just port line into orbatCreatorOnFaction function
        [_logic,_operation,_args] call MAINCLASS;
    ...
    case "openInterface": {
        ...
        private _interfaceName = _args;

        private _state = [_logic,"state"] call MAINCLASS;

        switch (_interfaceName) do {
...
=======
            ...
            case "Copy_Faction": {
                createDialog "ALiVE_orbatCreator_interface_copyFaction";
            };
...
=======
        ...
        [_state,"activeInteface", _interfaceName] call ALiVE_fnc_hashSet;

    };

    case "loadFactionDataFromConfig": {
...

        {
            ...
            if (_factionSide >= 0 && { _factionSide <= 3 }) then {
                ...
                private _factionGroupCategoriesData = [_logic,"initFactionGroupCategories", _factionGroupCategories] call MAINCLASS;
    ...
    case "onLoad": {
        ...
        _args params ["_interfaceName","_display"];
        ...
        switch (_interfaceName) do {
            ...
            case "Faction_Editor": {
                ...
                [_logic,"loadFactionsToList", [_factionList]] call MAINCLASS;

                // onSel event handlers are applied next frame
                private _selectedFaction = [_state,"selectedFaction"] call ALiVE_fnc_hashGet;
                [{
                    params ["_selectedFaction","_factionList"];
                ...
                }, [_selectedFaction, _factionList]] call CBA_fnc_execNextFrame;
>>>>>>> Stashed changes
...
=======
            case "Create_Faction": {
                ...
                [_logic,"loadFlagsToList", [_inputFlag]] call MAINCLASS;
...
=======
                [_logic,"onLoad", ["Edit_Faction",_display]] call MAINCLASS;

                _display displayAddEventHandler ["unload", "['onUnload', ['Copy_Faction',_this]] call ALiVE_fnc_orbatCreatorOnAction"];
...
=======
            ...
            case "Create_Group": {
                ...
                switch (_factionSide) do {
                    case 0: {
                        ...
                        _sideColor = ["EAST"] call ALiVE_fnc_sideTextToColor;
                        _sidePrefix = 'o';
                    };
                    case 1: {
                        _sideMarkerClass = "NATO_BLUFOR";
                        _sideColor = ["WEST"] call ALiVE_fnc_sideTextToColor;
                        _sidePrefix = 'b';
                    };
                    case 2: {
                        _sideMarkerClass = "NATO_Independent";
                        _sideColor = ["GUER"] call ALiVE_fnc_sideTextToColor;
                        _sidePrefix = 'n';
                    };
                };
    ...
    case "setFactionClassname": {
...

        {
            ...
            {
                [_x,"faction", _classname] call ALiVE_fnc_hashSet;
            } foreach (_groupsInCategory select 2);
        } foreach (_factionGroupCategories select 2);
    ...
    case "loadFactionsToList": {

        _args params ["_list",["_filter",{true}]];
...

        {
            ...
            if (!(_factionConfigName in FACTION_BLACKLIST) && { [_faction] call _filter }) then {
    ...
    case "getAllFlagsFromConfig": {

        private _allFlags = [];
        private _markerPath = configFile >> "CfgMarkers";

        // load flags from markers
    ...
    case "loadFlagsToList": {
        ...
        private _flags = [_logic,"getAllFlagsFromConfig"] call MAINCLASS;
        {
            _x params ["_name","_path"];

            if ([_name,_path] call _filter) then {
    ...
    case "onFactionEditorDeleteClicked": {
        ...
        private _deleteButton = _args select 0;
        private _display = ctrlParent _deleteButton;
        ...
        private _factionList = OC_getControl( _display , OC_FACTIONEDITOR_FACTIONS_LIST );
        private _indexToDelete = [_factionList,_faction] call ALiVE_fnc_listFindData;
        [_factionList,_indexToDelete] call ALiVE_fnc_listDelete;

    };

    case "onCreateFactionAutogenerateClassnameClicked": {

        private _generateClassnameButton = _args select 0;
        private _display = ctrlParent _generateClassnameButton;

        private _side = call compile OC_getSelData( OC_CREATEFACTION_INPUT_SIDE );
        private _country = ctrlText OC_getControl( _display , OC_CREATEFACTION_INPUT_COUNTRY );
        private _force = ctrlText OC_getControl( _display , OC_CREATEFACTION_INPUT_FORCE );
        private _camo = ctrlText OC_getControl( _display , OC_CREATEFACTION_INPUT_CAMO );

        if (_force != "") then {
            _country = format["%1%2",_country, _force];
        };

        private _generatedClassname = [_logic,"generateFactionClassname", [_side,_country,_camo]] call MAINCLASS;
    ...
    case "generateFactionClassname": {
        ...
        _args params ["_side","_country","_camo"];

        private _sideNum = [_logic,"convertSideToNum", _side] call MAINCLASS;
        private _sideText = [_sideNum] call ALiVE_fnc_sideNumberToText;
        private _sideTextLong = [_sideText] call ALiVE_fnc_sideTextToLong;
        _sideTextLong = [_sideTextLong] call CBA_fnc_trim;
        ...
        private _autogenClassname = format ["%1_%2", _sidePrefix, _countryPrefix];
    ...
    case "onCreateFactionOkClicked": {
        ...
        private _side = parsenumber OC_getSelData( OC_CREATEFACTION_INPUT_SIDE );
        ...
        private _fieldsAreValid = [_logic,"validationFactionFields", [_displayName,_className,_country]] call MAINCLASS;
        if !(_fieldsAreValid select 0) exitwith {
            private _context = OC_getControl( _display , OC_CREATEFACTION_CONTEXT );
            _context ctrlsettext (_fieldsAreValid select 1);
        };

        private _shortName = format["%1%2",_country,_force];
        ...
        private _factionEditorDisplay = finddisplay OC_DISPLAY_FACTIONEDITOR;
        private _factionList = OC_getControl( _factionEditorDisplay , OC_FACTIONEDITOR_FACTIONS_LIST );
        [_logic,"loadFactionsToList", [_factionList]] call MAINCLASS;
    ...
    case "onEditFactionOkClicked": {
        ...
        private _fieldsAreValid = [_logic,"validationFactionFields", [_newDisplayName,_newClassname,_country]] call MAINCLASS;
        if !(_fieldsAreValid select 0) exitwith {
            private _context = OC_getControl( _display , OC_EDITFACTION_CONTEXT );
            _context ctrlsettext (_fieldsAreValid select 1);
        };
    ...
    case "onCopyFactionOkClicked": {
        ...
        private _fieldsAreValid = [_logic,"validationFactionFields", [_displayName,_className,_country]] call MAINCLASS;
        if !(_fieldsAreValid select 0) exitwith {
            private _context = OC_getControl( _display , OC_COPYFACTION_CONTEXT );
            _context ctrlsettext (_fieldsAreValid select 1);
        };
        ...
        // copy units to new faction

        private _factionUnits = [];
        _factionUnits append ( [_logic,"getCustomUnitsByFaction", _faction] call MAINCLASS );

        private _assetsImportedFromConfig = [_factionData,"assetsImportedConfig"] call ALiVE_fnc_hashGet;
        if (!_assetsImportedFromConfig) then {
            private _unitsImportedFromConfig = [_logic,"importFactionUnitsFromConfig", _faction] call MAINCLASS;
            _factionUnits append _unitsImportedFromConfig;
        };
        ...
        private _factionEditorDisplay = finddisplay OC_DISPLAY_FACTIONEDITOR;
        private _listFactions = OC_getControl( _factionEditorDisplay , OC_FACTIONEDITOR_FACTIONS_LIST );
        [_logic,"loadFactionsToList", [_listFactions]] call MAINCLASS;
    ...
    case "validationFactionFields": {

        _args params ["_displayName","_className","_country"];

        private _state = [_logic,"state"] call MAINCLASS;
        private _factions = [_state,"factions"] call ALiVE_fnc_hashGet;

        if (_displayName == "") exitWith {
            _result = [false,"Display name cannot be left blank"];
        };

        if (_className == "") exitWith {
            _result = [false,"Class name cannot be left blank"];
        };

        if (_country == "") exitWith {
            _result = [false,"Country name cannot be left blank"];
        };

        if (_className in (_factions select 1)) exitWith {
            _result = [false,"A faction with that class name already exists"];
        };
