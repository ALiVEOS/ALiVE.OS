...
if (typename _config == "ARRAY") then { _config = [_config, configFile] call BIS_fnc_configPath; };

if (configName _config in _brokenCheckpoints) then { _azi = [_azi + 90] call ALiVE_fnc_modDegrees; };
...
if (str(_config) find "Furniture" != -1) then { _isFurniture = true; };
...
if (isNil QMOD(PCOMPOSITIONS)) then { MOD(PCOMPOSITIONS) = call ALiVE_fnc_hashCreate; };
