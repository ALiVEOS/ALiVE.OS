...
/* ----------------------------------------------------------------------------
Function: ALIVE_fnc_checkConfigWarnings
...
BOOL - Validate group membership for each unit / vehicle class
...
["BLU_F",true] call ALIVE_fnc_checkConfigWarnings;
...
for "_i" from 0 to count _factions -1 do {
    ...
    if(isClass _faction) then {
        ...
        if(_side < 3) then {
            [_configName, true] call ALIVE_fnc_checkConfigWarnings;
        };
    };
}
(end)
...
_validateGroupMembership = _this select 1;
...
//[""] call _dump;
[""] call _dump;
_text = " ----------- %1 Faction Compatibility ----------- ";
[_text, _faction] call _dump;
...
//[""] call _dump;
_text = " ----------- %1 CfgVehicles Men ----------- ";
[_text,_faction] call _dump;
//[""] call _dump;


if (_validateGroupMembership) then {
    {

        private _vehicleClass = _x;

        private _configName = configName _vehicleClass;

        // [_configName] call _dump;

        private _vehicleType = [_vehicleClass >> "vehicleClass"] call _cfgValue;

        //["-- Type: %1",_vehicleType] call _dump;

        /// Check to see if unit is used in CfgGroups at all
        ...
        if (count _factionGroups > 0) then {
            {
                for "_i" from 0 to count _x -1 do {
                        ...
                        private _class = _x select _i;

                        if (isClass _class) then {
                            private _unitVehicle = [_class >> "vehicle"] call _cfgValue;
                            if (_unitVehicle == _configName) exitWith {
                                _found = true;
                            };
                        };
        ...
        if !(_found) then {
            ["-- warning: Unit %1 is not included in any faction groups, suggest adding it!",_configName] call _dump;
        };
...
if (_validateGroupMembership) then {
    {
        ...
        private _vehicleClasses = [_factionVehicles,_vehicleType] call ALIVE_fnc_hashGet;
        {
            ...
            if !(_found) then {
                ["-- warning: Vehicle %1 is not included in any faction groups, suggest adding it!",_configName] call _dump;
            };
...
_spawnPosition = (getPosATL player) getPos [100, random 360];
...
[_factionVehicles,_side,_faction,_spawnPosition] spawn {
...

    {
...

        {
            ...
            _spawnPosition = _spawnPosition getPos [100, random 360];
